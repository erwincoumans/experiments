...
int main2() {
 ...
 FILE* fp;
  fp = fopen("D:/PhysicsAnimationBakingDemo.blend","rb");
  fseek(fp, 0L, SEEK_END);
	int mFileLen = ftell(fp);
	fseek(fp, 0L, SEEK_SET);

	char* mFileBuffer = ( char*)malloc(mFileLen+1);
  ...
  fprintf(fileWrite,"const char* mydata_base64 = {\n");
	...
	{
		fprintf(fileWrite,"%d,",encoded.c_str()[i]);
		counter++;
		if (counter>50)
		{
			counter=0;
  ...
  std::string decoded = base64_decode(bla);
  {
  FILE* myfout = fopen("d:/binnetje.bin","wb");
  fwrite(decoded.c_str(), decoded.size(),1,myfout);
  fclose(myfout);
  }
