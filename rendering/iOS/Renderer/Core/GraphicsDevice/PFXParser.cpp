...
public:
	CPFXParserReadContext();
	~CPFXParserReadContext();
};

/*!***************************************************************************
 @Function			CPFXParserReadContext			
 @Description		Initialises values.
*****************************************************************************/
CPFXParserReadContext::CPFXParserReadContext()
{
	nMaxLines = 5000;
	nNumLines = 0;
...
/*!***************************************************************************
 ...
 @Description		
*****************************************************************************/									
bool GetSemanticDataFromString(SSemanticDefaultData *pDataItem, const char * const pszArgumentString, ESemanticDefaultDataType eType, string *pError)
{
	...
	if(pszString[0] != '(')
	{
		*pError = string("Missing '(' after ");
                *pError += c_psSemanticDefaultDataTypeInfo[eType].pszName;
		return false;
	}
	pszString++;
	...
	if(!strlen(pszString))
	{
		*pError = c_psSemanticDefaultDataTypeInfo[eType].pszName + string(" missing arguments");
		return false;
	}
	...
	if(pszString == pszTmp)
	{
		...
		*pError = string("'") + pszError + "' unexpected for " + c_psSemanticDefaultDataTypeInfo[eType].pszName;
		delete pszError;
		return false;
	}
	pszString = pszTmp;
	...
	if(pszString[0] != ')')
	{
		...
		strcpy(pszError, "");
		strncat(pszError, pszString, n);
		*pError = string("'") + pszError + "' found when expecting ')' for " + c_psSemanticDefaultDataTypeInfo[eType].pszName;
		delete pszError;	
		return false;
	}
	pszString++;
	...
	if(strlen(pszString))
	{
		*pError = string("'") + pszString + "' unexpected after ')'";
		return false;
	}
...
CPFXParser::CPFXParser()
{
	...
	m_nMaxTextures = 10;
	m_nNumTextures = 0;
	m_psTexture = new SPFXParserTexture[m_nMaxTextures];

	m_nMaxVertShaders = 10;
	m_nNumVertShaders = 0;
	m_psVertexShader = new SPFXParserShader[m_nMaxVertShaders];

	m_nMaxFragShaders = 10;
	m_nNumFragShaders = 0;
	m_psFragmentShader = new SPFXParserShader[m_nMaxFragShaders];

	m_nMaxEffects = 10;
	m_nNumEffects = 0;
	m_psEffect = new SPFXParserEffect[m_nMaxEffects];
}
...
CPFXParser::~CPFXParser()
{
	...
	// free header strings
	delete m_sHeader.pszVersion;
	delete m_sHeader.pszDescription;
	delete m_sHeader.pszCopyright;

	// free texture info
	for(i = 0; i < m_nNumTextures; ++i)
	{
		delete m_psTexture[i].pszName;
		delete m_psTexture[i].pszFile;
	}
	delete [] m_psTexture;
	...
	{
		delete m_psFragmentShader[i].pszName;
		delete m_psFragmentShader[i].pszGLSLfile;
		delete m_psFragmentShader[i].pszGLSLcode;
		delete m_psFragmentShader[i].pszGLSLBinaryFile;
		delete m_psFragmentShader[i].pbGLSLBinary;
	}
	delete [] m_psFragmentShader;

	for(i = 0; i < m_nNumVertShaders; ++i)
	{
		delete m_psVertexShader[i].pszName;
		delete m_psVertexShader[i].pszGLSLfile;
		delete m_psVertexShader[i].pszGLSLcode;
		delete m_psVertexShader[i].pszGLSLBinaryFile;
		delete m_psVertexShader[i].pbGLSLBinary;
	}
	...
	for(unsigned int nEffect = 0; nEffect < m_nNumEffects; ++nEffect)
	{
		...
		{
			delete m_psEffect[nEffect].psUniform[i].pszName;
			delete m_psEffect[nEffect].psUniform[i].pszValue;
		}
		...
		{
			delete m_psEffect[nEffect].psAttribute[i].pszName;
			delete m_psEffect[nEffect].psAttribute[i].pszValue;
		}
		delete [] m_psEffect[nEffect].psAttribute;

		for(i=0; i < m_psEffect[nEffect].nNumTextures; ++i)
		{
			delete m_psEffect[nEffect].psTextures[i].pszName;
		}
		delete [] m_psEffect[nEffect].psTextures;

		delete m_psEffect[nEffect].pszFragmentShaderName;
		delete m_psEffect[nEffect].pszVertexShaderName;

		delete m_psEffect[nEffect].pszAnnotation;
		delete m_psEffect[nEffect].pszName;
	}
	delete [] m_psEffect;
}
...
*****************************************************************************/
bool CPFXParser::Parse(string * const pReturnError)
{
	int nEndLine = 0;
	...
	for(unsigned int nLine=0; nLine < m_psContext->nNumLines; nLine++)
	{
		...
		else if(strcmp("[VERTEXSHADER]", m_psContext->ppszEffectFile[nLine]) == 0)
		{
			if(GetEndTag("VERTEXSHADER", nLine, &nEndLine))
			{
				...
				{
					delete m_psVertexShader[m_nNumVertShaders].pszName;
					delete m_psVertexShader[m_nNumVertShaders].pszGLSLfile;
					delete m_psVertexShader[m_nNumVertShaders].pszGLSLcode;
					delete m_psVertexShader[m_nNumVertShaders].pszGLSLBinaryFile;
					return false;
				}
			}
			else
		...
		else if(strcmp("[FRAGMENTSHADER]", m_psContext->ppszEffectFile[nLine]) == 0)
		{
			if(GetEndTag("FRAGMENTSHADER", nLine, &nEndLine))
			{
				...
				{
					delete m_psFragmentShader[m_nNumFragShaders].pszName;
					delete m_psFragmentShader[m_nNumFragShaders].pszGLSLfile;
					delete m_psFragmentShader[m_nNumFragShaders].pszGLSLcode;
					delete m_psFragmentShader[m_nNumFragShaders].pszGLSLBinaryFile;
					return false;
				}
			}
			else
		...
		else if(strcmp("[EFFECT]", m_psContext->ppszEffectFile[nLine]) == 0)
		{
			if(GetEndTag("EFFECT", nLine, &nEndLine))
			{
				...
				else
				{
					...
					{
						delete m_psEffect[m_nNumEffects].psUniform[i].pszName;
						delete m_psEffect[m_nNumEffects].psUniform[i].pszValue;
					}
					delete [] m_psEffect[m_nNumEffects].psUniform;

					for(i=0; i < m_psEffect[m_nNumEffects].nNumAttributes; ++i)
					{
						delete m_psEffect[m_nNumEffects].psAttribute[i].pszName;
						delete m_psEffect[m_nNumEffects].psAttribute[i].pszValue;
					}
					delete [] m_psEffect[m_nNumEffects].psAttribute;

					for(i=0; i < m_psEffect[m_nNumEffects].nNumTextures; ++i)
					{
						delete m_psEffect[m_nNumEffects].psTextures[i].pszName;
					}
					delete [] m_psEffect[m_nNumEffects].psTextures;

					delete m_psEffect[m_nNumEffects].pszFragmentShaderName;
					delete m_psEffect[m_nNumEffects].pszVertexShaderName;

					delete m_psEffect[m_nNumEffects].pszAnnotation;
					delete m_psEffect[m_nNumEffects].pszName;
...
*****************************************************************************/
unsigned int CPFXParser::ParseFromMemory(const char * const pszScript, string * const pReturnError)
{
	CPFXParserReadContext	context;
	char			pszLine[512];
	...
	return Parse(pReturnError) ? SUCCESS : FAIL;
}
...
/*!***************************************************************************
 ...
 @Output			pReturnError	error string
 @Return			unsigned int	SUCCESS for success parsing file
									FAIL if file doesn't exist or is invalid
 @Description		Reads the PFX file and calls the parser.
*****************************************************************************/
unsigned int CPFXParser::ParseFromFile(const char * const pszFileName, string * const pReturnError)
{
	CPVRTResourceFile PfxFile(pszFileName);
	if (!PfxFile.IsOpen())
	{
		*pReturnError = string("Unable to open file ") + pszFileName;
		return FAIL;
	}
...
*****************************************************************************/
bool CPFXParser::GetEndTag(char *pszTagName, int nStartLine, int *pnEndLine)
{
	char pszEndTag[100];
	strcpy(pszEndTag, "[/"); 
...
/*!***************************************************************************
 ...
 @Description		Resuces all white space characters in the string to one 
					blank space.
*****************************************************************************/
void CPFXParser::ReduceWhitespace(char *line)
{
...
*****************************************************************************/
bool CPFXParser::ParseHeader(int nStartLine, int nEndLine, string * const pReturnError)
{
...
*****************************************************************************/
bool CPFXParser::ParseTextures(int nStartLine, int nEndLine, string * const pReturnError)
{
	m_nNumTextures = 0;
...
/*!***************************************************************************
 ...
 @Description		Parses the VERTEXSHADER or FRAGMENTSHADER section of the 
					PFX file.
*****************************************************************************/
bool CPFXParser::ParseShader(int nStartLine, int nEndLine, string * const pReturnError, SPFXParserShader &shader, const char * const pszBlockName)
{
...
*****************************************************************************/
bool CPFXParser::ParseSemantic(SPFXParserSemantic &semantic, const int nStartLine, const int nEndLine, string * const pReturnError)
{
	char *str;
...
*****************************************************************************/
bool CPFXParser::ParseEffect(SPFXParserEffect &effect, const int nStartLine, const int nEndLine, string * const pReturnError)
{
	bool bName = false;
	...
	effect.psTextures				= new SPFXParserEffectTexture[effect.nMaxTextures];

	effect.nMaxUniforms				= 100;
	effect.nNumUniforms				= 0;
	effect.psUniform				= new SPFXParserSemantic[effect.nMaxUniforms];

	effect.nMaxAttributes			= 100;
	effect.nNumAttributes			= 0;
	effect.psAttribute				= new SPFXParserSemantic[effect.nMaxAttributes];
...
*****************************************************************************/
void CPFXParser::DebugDump() const
{
	unsigned int i;
	...
	printf("[GLSL_CODE]\n");
	printf(m_psVertexShader[0].pszGLSLcode);
	printf("[/GLSL_CODE]\n");
	printf("[/VERTEXSHADER]\n\n");
	...
	printf("[GLSL_CODE]\n");
	printf(m_psFragmentShader[0].pszGLSLcode);
	printf("[/GLSL_CODE]\n");
	printf("[/FRAGMENTSHADER]\n\n");
