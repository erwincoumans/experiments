...
#include "Shader.h"
#include "PFXParser.h"
#include "PFXParserAPI.h"
#include "Texture.h"

// Originally form glExt2.h in the PowerVR SDK, format to load binary shaders.
...
/*!***************************************************************************
 ...
 @Returns			EError			SUCCESS if load succeeded
 @Description		Loads the specified effect from the CPFXParser object.
					Compiles and links the shaders. Initialises texture data.
*****************************************************************************/
unsigned int CPFXEffect::Load(CPFXParser &src, const char * const pszEffect, const char * const pszFileName, string *pReturnError)
{
	...
	*/
	m_pParser = &src;
	SPFXParserEffect *psParserEffect = &src.m_psEffect[m_nEffect];

	// Create room for per-texture data
	m_psTextures = new SPFXTexture[src.m_nNumTextures];
	...
	// Initialise the effect
	{
		...
		{
			if (ShaderLoadBinaryFromMemory(src.m_psVertexShader[i].pbGLSLBinary, src.m_psVertexShader[i].nGLSLBinarySize,
												GL_VERTEX_SHADER, GL_SGX_BINARY_IMG, &uiVertexShader) == SUCCESS)
			{
		...
		if (bLoadSource)
		{
			if(pszVertexShader)
			{
				if (ShaderLoadSourceFromMemory(pszVertexShader, GL_VERTEX_SHADER, &uiVertexShader) != SUCCESS)
				{
					*pReturnError = string("Vertex Shader compile error in file '") + pszFileName + "':\n" + error;
					if(bFreeVertexShader)	delete pszVertexShader;
					if(bFreeFragmentShader)	delete pszFragmentShader;
					return FAIL;
				}
			}
			else // Shader not found or failed binary block
			{
				if (src.m_psVertexShader[i].pbGLSLBinary==NULL)
				{
					*pReturnError = string("Vertex shader ") + psParserEffect->pszVertexShaderName + "  not found in " + pszFileName + ".\n";
				}
				else
				{
					*pReturnError = string("Binary vertex shader ") + psParserEffect->pszVertexShaderName + " not supported.\n";
				}
		...
		{
			if (ShaderLoadBinaryFromMemory(src.m_psFragmentShader[i].pbGLSLBinary, src.m_psVertexShader[i].nGLSLBinarySize,
													GL_FRAGMENT_SHADER, GL_SGX_BINARY_IMG, &uiFragShader) == SUCCESS)
			{
				// success loading the binary block so we do not need to load the source
				bLoadSource = 0;
			}
		...
		if (bLoadSource)
		{
			...
			{
				if (ShaderLoadSourceFromMemory(pszFragmentShader, GL_FRAGMENT_SHADER, &uiFragShader) != SUCCESS)
				{
					*pReturnError = string("Fragment Shader compile error in file '") + pszFileName + "':\n" + error;
					if(bFreeVertexShader)	delete pszVertexShader;
					if(bFreeFragmentShader)	delete pszFragmentShader;
					return FAIL;
				}
			...
			else // Shader not found or failed binary block
			{
				if (src.m_psFragmentShader[i].pbGLSLBinary==NULL)
				{
					*pReturnError = string("Fragment shader ") + psParserEffect->pszFragmentShaderName + "  not found in " + pszFileName + ".\n";
				}
				else
				{
					*pReturnError = string("Binary Fragment shader ") + psParserEffect->pszFragmentShaderName + " not supported.\n";
				}
		...
		if (!Linked)
		{
			...
			*pReturnError = string("Error Linking shaders in file '") + pszFileName + "':\n\n"
							+ string("Failed to link: ") + pszInfoLog + "\n";
			delete [] pszInfoLog;
			return FAIL;
		}
		...
		*/
		...
		for(i = 0; i < psParserEffect->nNumTextures; ++i)
		{
			...
			{
				*pReturnError = string("Effect \"") +  psParserEffect->pszName + "\", requested non-existent texture: \""
...
/*!***************************************************************************
 @Function			Activate
 @Returns			SUCCESS if activate succeeded
 @Description		Selects the gl program object and binds the textures.
*****************************************************************************/
unsigned int CPFXEffect::Activate()
{
	unsigned int i;
	
	SPFXParserEffect *psParserEffect = &m_pParser->m_psEffect[m_nEffect];
...
static unsigned int GetSemantics(
	SPFXUniform					* const psUniforms,
	unsigned int					* const pnUnknownUniformCount,
	const SPFXParserSemantic	* const psParams,
	...
	string						* const errorMsg)
{
	...
	for(j = 0; j < nParamCount; ++j)
	{
		for(i = 0; i < nUniformSemantics; ++i)
		{
			...
			else
			{
				...
				*errorMsg += string(psParams[j].pszName) + " ";
				*errorMsg += string(psParams[j].pszValue) + "\n";
			}
	...
	*pnUnknownUniformCount	= nParamCount - nCount;
	return nCount;
}
...
/*!***************************************************************************
 ...
 @Output			pReturnError				error string
 @Returns			EError					SUCCESS if succeeded
 @Description		Builds the uniform table from the semantics.
*****************************************************************************/
unsigned int CPFXEffect::BuildUniformTable(
	...
	string							*pReturnError)
{
	...
	psUniforms = (SPFXUniform*)malloc(nCount * sizeof(*psUniforms));
	if(!psUniforms)
		return FAIL;
...
*****************************************************************************/
const SPFXTexture *CPFXEffect::GetTextureArray(unsigned int &nCount) const
{
...
*****************************************************************************/
void CPFXEffect::SetTexture(const unsigned int nIdx, const GLuint ui, const unsigned int u32flags)
{
...
*****************************************************************************/
void CPFXEffect::SetDefaultUniformValue(const char *const pszName, const SSemanticDefaultData *psDefaultValue)
{
