...
public:
    ...
    typedef typename MemoryManager::const_device_pointer const_device_pointer;
...
#undef VSWAP
    ...
    const_device_pointer deviceReadAt ( int i ,int gpu = MemoryManager::getBufferDevice()) const {
        copyToDevice(gpu);
		return MemoryManager::deviceOffset(devicePointer[gpu],i);
    }
    
    const_device_pointer deviceRead ( int gpu = MemoryManager::getBufferDevice()) const { return deviceReadAt(0,gpu); }
...
public:
    ...
    typedef CPUMemoryManager<T> MemoryManager;
    typedef typename MemoryManager::device_pointer device_pointer;
    typedef typename MemoryManager::const_device_pointer const_device_pointer;
	...
	/// Constructor
	const vector<T, MemoryManager>& operator=(const std::vector<T, Alloc>& x)
	{
...
public:
    ...
    const_device_pointer deviceReadAt ( int i=0, int gpu = MemoryManager::getBufferDevice() ) const {
        return &*(this->begin()+i);
    }
    
    device_pointer deviceWriteAt ( int i=0, int gpu = MemoryManager::getBufferDevice() ) {
        return &*(this->begin()+i);
    }
