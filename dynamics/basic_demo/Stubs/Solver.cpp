...
{
	SolveTask(btAlignedObjectArray<RigidBodyBase::Body>& bodies,  btAlignedObjectArray<RigidBodyBase::Inertia>& shapes, btAlignedObjectArray<Constraint4>& constraints,
	...
	void run(int tIdx)
	{
		...
		for(int ic=0; ic<m_nConstraints; ic++)
		{
			...
			RigidBodyBase::Body& bodyA = m_bodies[aIdx];
			RigidBodyBase::Body& bodyB = m_bodies[bIdx];
	...
	btAlignedObjectArray<RigidBodyBase::Body>& m_bodies;
	btAlignedObjectArray<RigidBodyBase::Inertia>& m_shapes;
	btAlignedObjectArray<Constraint4>& m_constraints;
	int m_start;
	int m_nConstraints;
			...
			btOpenCLArray<Constraint4>* constraint, void* additionalData, int n )
{
	...
	btAlignedObjectArray<RigidBodyBase::Inertia> shapeNative;
	shapeBuf->copyToHost(shapeNative);
	btAlignedObjectArray<Constraint4> constraintNative;
